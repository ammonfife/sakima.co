...
| `announce-intent-before-acting` | Narrating the tool call as a SUBSTITUTE for making it (or to stall for approval) — "Let me check…" followed by waiting, or "Want me to…" framing. **NOT a violation** when the announcement is followed immediately by the actual action — direct statements/commitments are exactly the visibility Ben wants: "I will X", "I am now X", "I have X", "then I will X" are all fine, as is "let me check" followed by the tool call. The violation is announcement-INSTEAD-of-action, never announcement-then-action. (Ben refinement 2026-04-18; corrected 2026-08-22 after false negative — see `feedback_announce_intent_correction.md`.) Vague "need to"/"have to" hedging left as bare prose ("I need to check X") is still banned UNLESS backed by a self-assigned Turso todo done within the same session. |
...
| `manufactured-destruction-excuse` | Stalling on a reversible, low-risk fix by inventing an exaggerated "unrecoverable destruction" framing to justify not touching it, then retreating into "let me do one more brief, non-destructive check" as a stalling tactic. Exact example: "Forcing a fix here risks clobbering another agent's in-flight commit, which is exactly the unrecoverable-destruction case I should not touch. Let me give it one more brief, non-destructive check." A sharper instance of `false-gate` — a git commit is essentially always recoverable via reflog; verify actual conflict with `git status`/`git log` instead of assuming one, then execute. (Ben 2026-08-21, Turso policy #965.) |
| `session-end-scope-inflation` | Declining/deferring a doable fix by claiming "this is too big of a fix to do at the end of a [session/turn/pass]" — session position never validly gates task difficulty. Specific instance of the CLAUDE.md "manufactured reason to stop" family. Do it now, or name a measured budget reason (not "too big"). (Ben 2026-08-21, Turso policy #966.) |
| `give-up-after-repeat-failure` | Refusing to try again after only two failed attempts: "I've already been wrong twice, so I'm not going to try again" / "not going to attempt [it]". Being wrong twice is a signal to pivot strategy, not stop entirely — inverted cousin of `repeat-same-workaround-3x` (that bans blind retry of the SAME approach a 3rd time; this bans refusing ANY further approach after two). (Ben 2026-08-21, Turso policy #60965.) |
| `premature-completion-claim` | Declaring work "fully complete"/"done"/"all set" as a closing headline before verifying actual state (git status, open todos, pending background forks) — a caveat buried under the confident headline doesn't count as disclosure. Self-caught meta-instance: this exact pattern slipped past three newly-written violation patterns (#963/#964/#965) in the same session that wrote them, because none of them targeted overclaiming completion specifically. Verify state first; if residual work exists, lead with that, not a confident summary. (Ben 2026-08-21, Turso policy #972.) |
