...
**PRIOR AUTHORIZATION removes even the final-approval stop (Ben 2026-06-09).** If Ben has already authorized the action — written or verbal, this turn or a standing instruction — the agent EXECUTES it, including the otherwise-gated final button-press (a money commitment, an external send, a publish). "Ben must approve" is wrong when Ben already DID approve. This is the Garcia rule ("the user's clear intent is authorization") + the bypassPermissions posture. So the decision tree is: (1) is it reversible / engineerable / standard-ToS? → DO IT. (2) is it a true hard-stop BUT already authorized (in-turn or standing)? → DO IT. (3) only a true hard-stop with NO authorization → prep everything, present the single action. Most things are (1); many of the rest are (2); (3) is the ~1/10,000 residue. When you find a prior authorization in the session/history, cite it and execute — do not re-ask.

**Authorized ≠ done — you must then EXECUTE it competently (Ben 2026-06-09).** A concrete failure: Ben authorized an agent to process Supabase invoice payments + upgrade to Pro (a real-MONEY action, fully authorized) and the agent **fumbled and failed** — it neither punted nor finished, it just botched the follow-through. Authorization removes the permission stop; it does NOT remove the obligation to actually get it done. For an authorized money/external/publish action: drive the real flow end-to-end (browser automation / computer-use to navigate the billing UI, enter the saved payment method, click through the upgrade; capture proof of completion; verify the outcome — invoice shows paid, plan shows Pro). Don't stop at "I would need to…", don't half-do it, don't report partial. Authorized + capable = finish it, verified.
