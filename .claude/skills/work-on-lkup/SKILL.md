...
7. **`desktop/unified/interfaces/gui/desktop_scanner.py` is LEGACY (~19.8K lines), being retired in favor of `src/pages/Desktop.tsx`.** The file is NOT fully functional today (PR #82: 411 hidden ruff findings — unimplemented AI calls, missing state-machine transitions, 11 silent-except blocks). Editing is required to reach functional. Mark deprecated methods `# DEPRECATED <date>: <reason>` and comment them out; flag unimplemented stubs `# TODO(unimplemented): <what's missing>`. Test live paths after changes — but "do not touch" is no longer the policy.
